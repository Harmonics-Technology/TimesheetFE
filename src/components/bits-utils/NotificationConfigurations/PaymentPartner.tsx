...
import {
    PaymentPartnerSettingModel,
    PaymentPartnerSettingView,
    UserSettingService,
} from 'src/services';
...
export const PaymentNotificationConfiguration = ({
    controls,
}: {
    controls: PaymentPartnerSettingView;
}) => {
    const [access, setAccess] = useState<PaymentPartnerSettingModel>({
    ...
    const updateControl = async (data: PaymentPartnerSettingModel) => {
        setLoading(true);
        try {
            const result =
                await UserSettingService.updatePaymentPartnerSettings(data);
            if (result.status) {
                setLoading(false);
                toast({
